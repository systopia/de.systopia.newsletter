...
class CRM_Newsletter_RegisterTokenFlexmailer {
  ...
  public static function register_tokens(): void {
    ...
    foreach (CRM_Newsletter_Profile::getProfiles() as $profile_name => $profile) {
      $additional_profiles['newsletter.optin_url_' . $profile_name]
        = E::ts('Newsletter Profile Link from de.systopia.newsletter for profile %1', [1 => $profile_name]);
      $additional_profiles['newsletter.preferences_url_' . $profile_name]
        = E::ts('Newsletter Profile Link from de.systopia.newsletter for profile %1', [1 => $profile_name]);
    }
